...
namespace WeatherLink.Models
{
    ...
    public class WeatherBasedTrafficAdvice
    {
        ...
        /// <summary>
        ///     Gets or sets the next point in time that has measurable precipitation after the next minimum level of
        ///     precipitation.
        /// </summary>
        ...
        public override string ToString()
        {
            ...
            if (Currently.PrecipIntensity > 0)
            {
                ...
                {
                    sb.AppendLine(
                        $"The best time to leave in the next hour is {BestTimeToLeave.DateTime.Humanize(homeDateTimeOffset)}.");
                }

                if (MinimumPrecipitation != null)
                {
                    sb.AppendLine(
                        $"{Currently.PrecipType.ToString().Transform(To.SentenceCase)} reaches lowest point {MinimumPrecipitation.DateTime.Humanize(homeDateTimeOffset)}.");
                }

                if (NextModeratePrecipitation != null)
                {
                    var precipType = NextModeratePrecipitation.PrecipType == PrecipitationType.None
                        ? Currently.PrecipType
                        : NextModeratePrecipitation.PrecipType;
                    sb.AppendLine(
                        $"{precipType.ToString().Transform(To.SentenceCase)} getting worse {NextModeratePrecipitation.DateTime.Humanize(homeDateTimeOffset)}.");
                }

                if (NextHeavyPrecipitation != null)
                {
                    var precipType = NextHeavyPrecipitation.PrecipType == PrecipitationType.None
                        ? Currently.PrecipType
                        : NextHeavyPrecipitation.PrecipType;
                    sb.AppendLine(
                        $"{precipType.ToString().Transform(To.SentenceCase)} getting much worse {NextHeavyPrecipitation.DateTime.Humanize(homeDateTimeOffset)}.");
                }

                if (NextPrecipitationAfterMinimum != null)
                {
                    var precipType = NextPrecipitationAfterMinimum.PrecipType == PrecipitationType.None
                        ? Currently.PrecipType
                        : NextPrecipitationAfterMinimum.PrecipType;
                    sb.AppendLine(
                        $"{precipType.ToString().Transform(To.SentenceCase)} starting again after lowest point {NextPrecipitationAfterMinimum.DateTime.Humanize(homeDateTimeOffset)}.");
                }
            }
            else
            {
                ...
                {
                    sb.AppendLine(
                        $"Next light {NextPrecipitation.PrecipType} is {NextPrecipitation.DateTime.Humanize(homeDateTimeOffset)}.");
                }

                if (NextModeratePrecipitation?.PrecipType != null)
                {
                    sb.AppendLine(
                        $"Next moderate {NextModeratePrecipitation.PrecipType} is {NextModeratePrecipitation.DateTime.Humanize(homeDateTimeOffset)}.");
                }

                if (NextHeavyPrecipitation?.PrecipType != null)
                {
                    sb.AppendLine(
                        $"Next heavy {NextHeavyPrecipitation.PrecipType} is {NextHeavyPrecipitation.DateTime.Humanize(homeDateTimeOffset)}.");
                }
            }
